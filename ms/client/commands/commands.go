...
type Command struct {
	...
	Security   SubSecurity   `xml:"security,omitempty"`     // используем без указателя
	Filter     string        `xml:"filter,attr,omitempty"`  // добавляем поле
	TradeNo    string        `xml:"tradeno,attr,omitempty"` // добавляем поле
	AllTrades  SubAllTrades  `xml:"alltrades,omitempty"`
	Quotations []SubSecurity `xml:"quotations,omitempty"`
	Quotes     []SubSecurity `xml:"quotes,omitempty"`
}
...
type SubSecurity struct {
	...
	Board   string `xml:"board,omitempty"`   // добавляем поле
	SecCode string `xml:"seccode,omitempty"` // добавляем поле
	//Security struct {
...
// Структура для подписки на тиковые данные
type SubscribeTicks struct {
	XMLName  xml.Name        `xml:"command"`
	Id       string          `xml:"id,attr"` // id="subscribe_ticks"
	Security SubTickSecurity `xml:"security,omitempty"`
	Filter   string          `xml:"filter,attr,omitempty"` // true/false - фильтр периода торгов
}

// Структура с информацией о бумаге для подписки на тики
type SubTickSecurity struct {
	Board   string `xml:"board"`             // Идентификатор режима торгов
	SecCode string `xml:"seccode"`           // Код инструмента
	TradeNo int64  `xml:"tradeno,omitempty"` // Номер сделки, после которой получаем тики
}

// Структура для списка тиков
type Ticks struct {
	XMLName xml.Name `xml:"ticks"`
	Items   []Tick   `xml:"tick"`
}

// Структура для отдельного тика
type Tick struct {
	XMLName      xml.Name `xml:"tick"`
	SecId        int      `xml:"secid"`        // Идентификатор бумаги
	TradeNo      int64    `xml:"tradeno"`      // Номер сделки
	TradeTime    string   `xml:"tradetime"`    // Время сделки
	Price        float64  `xml:"price"`        // Цена сделки
	Quantity     int      `xml:"quantity"`     // Количество лотов
	Period       string   `xml:"period"`       // Торговый период (O - открытие, N - торги, С - закрытие)
	BuySell      string   `xml:"buysell"`      // Направление сделки (B/S)
	OpenInterest int      `xml:"openinterest"` // Количество открытых позиций
	Board        string   `xml:"board"`        // Идентификатор режима торгов
	SecCode      string   `xml:"seccode"`      // Код инструмента
}
